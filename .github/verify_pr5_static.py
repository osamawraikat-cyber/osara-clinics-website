...
S=requests.Session(); S.headers['User-Agent']='OSara-PR5-validation/2.0'
errors=[]; rep={'routes':{},'directions':{},'schema_geo':{},'variants':{},'broken_links':[],'broken_images':[],'hours':None}
all_internal=set()
for r in ROUTES:
    resp=S.get(PRE+r,timeout=25); soup=BeautifulSoup(resp.text,'html.parser')
    ...
    if soup.find_all('h1') and len(soup.find_all('h1'))!=1: errors.append(f'{r} H1 count {len(soup.find_all("h1"))}')
    if '32.062463' in resp.text or '35.864789' in resp.text: errors.append(f'{r} old coordinates remain in rendered HTML')
    for sc in soup.find_all('script',type='application/ld+json'):
        ...
        while stack:
            ...
            if isinstance(x,dict):
                ...
                if r=='/' and x.get('@type')=='MedicalClinic':
                    rep['hours']=x.get('openingHoursSpecification')
...
for href in sorted(all_internal):
    ...
    rr=S.get(PRE+href,allow_redirects=True,timeout=25)
...
rob=S.get(PRE+'/robots.txt',timeout=25).text; prodrob=S.get(PROD+'/robots.txt',timeout=25).text
rep['robots']=rob
if rob!=prodrob: errors.append('robots changed')
for r in ROUTES[1:]:
    for suffix in ['.html','/']:
        pth=r+suffix; a=S.get(PRE+pth,allow_redirects=False,timeout=25); b=S.get(PROD+pth,allow_redirects=False,timeout=25)
