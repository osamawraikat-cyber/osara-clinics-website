...
# Correct only personal Dr. Osama descriptions.
for p in htmls:
    ...
    if p.as_posix()=='doctors/dr-osama-alwreikat.html':
        ...
        s=s.replace('طبيب جلدية وتجميل في عيادات أوسارا، أبو نصير، عمّان. تعرّف', 'جلدية وتناسلية وليزر في عيادات أوسارا، أبو نصير، عمّان. تعرّف')
    ...
    # Replace all known coordinate-based patient directions links with authoritative listing.
    s=s.replace('https://www.google.com/maps/search/?api=1&query=32.062463,35.864789', MAP)
    p.write_text(s,encoding='utf-8')

# Homepage: correction + replace interactive embed with a fixed clinic-focused location card.
idx=Path('index.html')
...
# Shared CTA stylesheet on homepage too.
if '/assets/contact-cta.css' not in s:
    s=s.replace('</head>','    <link rel="stylesheet" href="/assets/contact-cta.css">\n</head>',1)
idx.write_text(s,encoding='utf-8')

# Add shared CTA stylesheet to all non-home pages and a clear contact block above the footer.
for p in htmls:
    ...
    if 'class="osara-contact-block"' not in s:
        ...
        # Prefer inside main so the design remains integrated.
        s=s.replace('</main>',block+'</main>',1)
    p.write_text(s,encoding='utf-8')

# Make all remaining patient-facing map links point to the authoritative listing.
for p in htmls:
    s=p.read_text(encoding='utf-8')
