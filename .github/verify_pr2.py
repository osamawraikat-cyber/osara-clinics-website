...
BASE='https://deploy-preview-2--gregarious-malabi-0dc7e1.netlify.app'
...
EXPECTED={r:PROD+r for r in ROUTES}
EXPECTED['/']=PROD+'/'
s=requests.Session(); s.headers['User-Agent']='OSara-PR2-validator/1.0'
...
for r in ROUTES:
    ...
    if len(h1)!=1: errors.append(f'{r} h1 count {len(h1)}')
...
# Unique metadata on the two new pages.
for field,data in [('title',titles),('description',descs)]:
    vals=[v for _,v in data]
    if len(vals)!=len(set(vals)): errors.append(f'duplicate {field} found across canonical set')

# New page entity relationships.
for r in ['/psoriasis-treatment','/acne-scar-treatment']:
    text=pages[r].get_text(' ',strip=True)
...
sm=s.get(BASE+'/sitemap.xml',timeout=20)
locs=re.findall(r'<loc>(.*?)</loc>',sm.text)
expected_locs=[EXPECTED[r] for r in ROUTES]
if sm.status_code!=200 or locs!=expected_locs: errors.append(f'sitemap mismatch: {locs}')

# Internal links must resolve on preview; external/tel/mail ignored.
seen=set()
for r,soup in pages.items():
    for a in soup.find_all('a',href=True):
        ...
        seen.add(path)
        rr=s.get(BASE+path,allow_redirects=True,timeout=20)
        if rr.status_code>=400: errors.append(f'broken internal {r} -> {path}: {rr.status_code}')

# Required link architecture.
req={
 '/dermatology':['/psoriasis-treatment','/acne-scar-treatment','/vitiligo-jordan','/botox-hyperhidrosis','/mole-removal'],
 '/':['/psoriasis-treatment','/acne-scar-treatment'],
 '/vitiligo-jordan':['/dermatology','/psoriasis-treatment','/acne-scar-treatment'],
 '/botox-hyperhidrosis':['/dermatology','/psoriasis-treatment','/acne-scar-treatment'],
 '/mole-removal':['/dermatology','/psoriasis-treatment','/acne-scar-treatment']}
...
# Production remained untouched and existing routes are healthy.
for r in EXISTING:
...
# www behavior on current production.
www=s.get('https://www.osaraclinics.com/',allow_redirects=False,timeout=20)
if www.status_code not in (301,302,307,308) or not www.headers.get('location','').startswith('https://osaraclinics.com'):
    errors.append(f'www redirect unexpected {www.status_code} {www.headers.get("location")}')

report={'routes':ROUTES,'titles':titles,'sitemap_count':len(locs),'internal_paths_checked':sorted(seen),'errors':errors}
open('http-report.json','w').write(json.dumps(report,ensure_ascii=False,indent=2))
