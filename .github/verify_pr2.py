...
for r in ROUTES:
    ...
    d=soup.find('meta',attrs={'name':'description'}); desc=d.get('content','') if d else ''
    c=soup.find('link',rel='canonical'); canonical=c.get('href') if c else None
    o=soup.find('meta',attrs={'property':'og:url'}); og=o.get('content') if o else None
    h1=soup.find_all('h1'); rb=soup.find('meta',attrs={'name':'robots'}); robots=rb.get('content','') if rb else ''
    ...
    if r in ['/psoriasis-treatment','/acne-scar-treatment','/vitiligo-jordan','/botox-hyperhidrosis','/mole-removal','/school-health']:
